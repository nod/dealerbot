...
class Tourney:
    ...
    def incmd(self, cmd):
        ...
        if (c in gamecmds or c in setupcmds) and (p == None or p.busted):
            self.privout(pid, 'You must be in the game to use the %s command.' % cmd.cmd)

        # If you're all in, you're commited
        elif c in actions and p.allin:
            self.privout(pid, "You're all in.  Command ignored.")
        
        # If you've folded, I'm going to ignore you
        elif c in actions and p.folded:
            self.privout(pid, "You're not involved in this hand.  %s command ignored." % c)
        ...
        # after joining and before the game begins.
            ...
            if c == 'QUIT':
                ...
                if not p.vacation:
                    self.privout(pid, "You're not on vacation.  Command ignored.")
                else:
                    p.vacation = False
        ...
        #####                        break
            ...
            if not self.playing:
                ...
                self.privout(pid, 'Board:      %s' % self.printboard(self.round, self.color))
        elif c == 'POSITION':
            if not self.playing:
        ...
        #                                                        self.position(p)))
                self.privout(pid, 'players:%2d position:%2d' % (self.nlive(),
                                                                p.position))
        elif c == 'STACK':
            if not self.playing:
                self.privout(pid, 'There is currently no game.')
            else:
                self.privout(pid, 'bankroll:%d  in play:%d' % (p.bankroll, p.inplay))
            ...
            if len(self.players) < 2:
                self.pubout('I need at least two players to start a tournament.')
            else:
                self.playing = True
            ...
            self.privout(pid, 'Your hole cards are: %s' % (p.hand.showhole(self.color),))
            ...
            p.cmd.cmd = 'NOOP'
            p.cmd.arg = ''
            self.privout(pid, 'Advance action cancelled.')
    ...
    def joingame(self, nick):
        ...
        self.privout(nick, 'Welcome to the game.')
    ...
    def prettyprint(self, pid):
        ...
        self.privout(pid, '+-+------------+--------+--------+------+----+--------+')
        self.privout(pid, '|#|   Name     |Bankroll| Action |Status|Pot#|Pot Size|')
        self.privout(pid, '+-+------------+--------+--------+------+----+--------+')
        for p in self.players:
            if p.busted:
                continue
    ...
    def newhand(self):
        ...
        if self.nlive() == self.nonvacation():
            ...
            for p in self.players:
                if not p.busted:
                    self.privout(p.nick, 'Your hole cards are: %s' %\
    ...
    def dohelp(self, pid):
        log.logger.debug('in dohelp()')
        self.pubout('see http://33ad.org/pb/cC3')
        """
    ...
    def doautofold(self, p):
        ...
        if p.autofold:
            self.privout(p.nick, "Okay.  Fold means fold.");
        else:
            self.privout(p.nick, "Okay.  You will fold if there's a bet to you, but check otherwise.")
